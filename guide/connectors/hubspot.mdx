...
The HubSpot connector links a HubSpot portal to Syntaxia. Use it to bring CRM, marketing, and deal data into your golden layer. The connector authenticates with HubSpot via OAuth, so you do not need to create or manage an API key yourself.
...
<Steps>
  <Step title="Add the source">
    From Source Management in the Data pillar, click **Add Source**, expand the **HubSpot** card, and enter a connection label like `HubSpot - Marketing` or `HubSpot - Acme`.
  </Step>
...
If your portal admin restricts OAuth installs, ask them to approve the Syntaxia app once. After that, any member who holds the Data pillar can reconnect without further admin intervention.
