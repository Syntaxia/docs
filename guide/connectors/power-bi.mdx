...
The Power BI connector links a Microsoft Power BI tenant to Syntaxia. Use it to bring workspaces, datasets, reports, dashboards, and dataflow metadata into Syntaxia so analytics structure can be reconciled alongside CRM data.

<Info>
Power BI is currently available as a data connector. Mapping Power BI artifacts onto the golden layer is on the roadmap. You can connect today and your data will be staged in the pipeline. Model coverage will be enabled in a future release.
</Info>
...
<Steps>
  ...
  <Step title="Enhance admin API responses with detailed metadata">
    ...
    These settings let Syntaxia read dataset schema details that will feed the golden layer.
  </Step>
</Steps>
...
At minimum, add the security group as a **Member** of the workspaces you plan to include in Syntaxia.

## Connect Power BI in Syntaxia

<Steps>
  <Step title="Add the source">
    From Source Management in the Data pillar, click **Add Source**, expand the **Power BI** card, and enter a connection label like `Power BI - Finance` or `Power BI - GTM`.
  </Step>
