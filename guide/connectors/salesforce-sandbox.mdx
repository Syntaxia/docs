...
The Salesforce Sandbox connector links a non-production Salesforce environment to Syntaxia. Use it to validate mapping changes, dry-run migrations, or train your team without touching production data. Sandbox connections authenticate against `test.salesforce.com` rather than `login.salesforce.com`, so they are kept on a separate connector to avoid accidental crossover.
...
- The sandbox user can authorize the connected app: either the app is installed in the sandbox, or the user has the **Approve Uninstalled Connected Apps** permission. The [connected app access](/guide/connectors/salesforce#connected-app-access) requirement applies to sandboxes the same as to production.
...
<Steps>
  <Step title="Add the source">
    From Source Management in the Data pillar, click **Add Source**, expand the **Salesforce Sandbox** card, and enter a connection label that names the sandbox, for example `Salesforce - UAT` or `Salesforce - DevSandbox`.
  </Step>
...
- **Test mapping changes**: connect a partial copy sandbox, review it in the [Golden Layer](/guide/golden-layer), and confirm the result before promoting to production.
...
<AccordionGroup>
  ...
  <Accordion title="Salesforce shows `We can't authorize you because of an OAuth error.`">
    Salesforce blocked the authorization because the connected app is not installed in the sandbox and the authorizing user cannot approve uninstalled connected apps. Resolve it with one of the options under [connected app access](/guide/connectors/salesforce#connected-app-access), then click **Reconnect**.
  </Accordion>
  <Accordion title="The `Approve Uninstalled Connected Apps` permission is not available in my sandbox">
    Sandboxes created before Salesforce's Summer '25 release may be missing the permission. Ask your Salesforce admin to run **Match Production Licenses to Sandbox without a Refresh**, or refresh the sandbox, and the permission becomes available.
  </Accordion>
