...
A **source** is one of your CRMs, like Salesforce or HubSpot, plugged into Syntaxia. Most sources connect through OAuth: you authorize the connection, name it, and let the first sync run. If you have a Salesforce export zip instead of connector access, you can [upload it](/guide/connectors/crm-export-upload) and skip OAuth entirely. Once the source card turns green, Syntaxia has mapped the source onto its model and you can review the result in the Golden Layer.
...
- You hold the **Data** pillar. Adding sources is a Data pillar action. See [Pillars](/guide/pillars) if you need to add yourself.
...
From Source Management in the Data pillar, click **Add Source**. The connector picker lists every supported source.
...
<Tip>
No connector access? The **Upload a CRM export** card at the top of the picker ingests a Salesforce export zip as a point-in-time snapshot, no OAuth required. See [Upload a CRM export](/guide/connectors/crm-export-upload). The rest of this guide covers the OAuth path.
</Tip>
...
<Columns cols={2}>
  <Card title="Review your Golden Layer" icon="layer-group" href="/get-started/golden-layer">
    See how your new source mapped onto the model and settle what needs a decision.
  </Card>
